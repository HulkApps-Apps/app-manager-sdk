        ...
        $response = appManagerCacheData($cacheKey, function () use ($request, $shopTableName, $storeFieldName, $planFieldName, $shopifyPlanFieldName, $cacheKey, $mostPopularPlanIds, $sdkVersions) {
            ...
            return [
                ...
                'has_active_charge' => (!empty($activeCharge['active_charge']))
                    || (!empty($plan) && (float) ($plan['price'] ?? 0) == 0)
                    || !$trialActivatedAt,
                'global_plan_charge' => !empty($plan)
                    && isset($plan['is_global'])
                    && $plan['is_global']
                    && (
                        !empty($activeCharge['active_charge'])
                        && !empty($activeCharge['bundle_charge'])
                    ),
            ];
        });
    ...
    public function users(Request $request) {
        ...
        $users = DB::table($tableName)
            ...
            ->when($search, function ($q) use ($shopify_fields, $search) {
                return $q->where(function ($inner) use ($shopify_fields, $search) {
                    $inner->where(($shopify_fields['name'] ?? 'name'), 'like', '%' . $search . '%')
